...
const login = async (req, res) => {
    ...
    try {
        ...
        let role=await db.query(`select role from st_role where id=?`,[user[0].role_id])
        console.log("role",role[0][0])

        res.status(200).json({ message: 'Login successful.', token,id:user[0].id,userName:user[0].user_name,role:role[0][0].role});
    } catch (err) {
