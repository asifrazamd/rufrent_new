...
// const [propType, setPropertyTypeList] = useState([]);
// const [balcony, setBalconyList] = useState([]);
// const [bathrooms, setBathroomList] = useState([]);
// const [bedroomList, setBedroomList] = useState([]);
// const [floorRangeList, setFloorRangeList] = useState([]);
// const [homeType, setHomeTypeList] = useState([]);
// const [parkingCount, setParkingCountList] = useState([]);
// const [propDesc, setPropertyDescriptionList] = useState([]);
// const [rentalRangeList, setRentalRangeList] = useState([]);
// const [tenant, setTenantTypeList] = useState([]);
// const [tenantEat, setTenantEatPreferenceList] = useState([]);

// Fetch data hook
//   const useFetchData = (tableName, fields, params = {}) => {
...
//         setError(null);
//         try {
//           const response = await getRecords(tableName, fields, params);
//           setData(response);
//         } catch (err) {
...
//       if (Object.values(params).every((param) => param !== undefined && param !== "")) {
//         fetchData();
//       } else {
//         setData([]);
//       }
...
//   const fetchDataComponents = (formData) => {
//     // State and City Fetch
//     const { data: stateList, isLoading: isLoadingState, error: stateError } = useFetchData(
//       "st_state",
//       "id,name"
//     );
  
//     const { data: cityList, isLoading: isLoadingCity, error: cityError } = useFetchData(
//       "st_city",
//       "id,name",
//       { state_id: formData.state }
//     );
  
//     // Builder and Community Fetch
//     const { data: builderList, isLoading: isLoadingBuilder, error: builderError } = useFetchData(
//       "st_builder",
//       "id,name",
//       { city_id: formData.city }
//     );
  
//     const { data: communityList, isLoading: isLoadingCommunity, error: communityError } = useFetchData(
//       "st_community",
//       "id,name",
...
//     // Static Data Fetch
//     const staticDataFetchConfigs = [
//       { tableName: "st_prop_type", stateSetter: "propType" },
//       { tableName: "st_balcony", stateSetter: "balcony" },
//       { tableName: "st_baths", stateSetter: "bathrooms" },
//       { tableName: "st_beds", stateSetter: "bedrooms" },
//       { tableName: "st_floor_range", stateSetter: "floorRange" },
//       { tableName: "st_home_type", stateSetter: "homeType" },
//       { tableName: "st_parking_count", stateSetter: "parkingCount" },
//       { tableName: "st_prop_desc", stateSetter: "propDesc" },
//       { tableName: "st_rental_range", stateSetter: "rentalRange" },
//       { tableName: "st_tenant", stateSetter: "tenant" },
//       { tableName: "st_tenant_eat_pref", stateSetter: "tenantEat" },
//     ];
  
//     const staticData = staticDataFetchConfigs.map((config) => {
//       const { data, isLoading, error } = useFetchData(config.tableName, "id,name");
//       return {
//         [config.stateSetter]: data,
//         [`isLoading${config.stateSetter.charAt(0).toUpperCase() + config.stateSetter.slice(1)}`]: isLoading,
//         [`${config.stateSetter}Error`]: error,
//       };
//     });
...
//     isLoadingCommunity,
//     communityError,
//     ...staticData.reduce((acc, curr) => ({ ...acc, ...curr }), {}),
//   };
// };
...
import tailwindStyles from "../utils/tailwindStyles";
import SearchableDropdown from "./SearchDropdownView";
import useFetchDataComponents from "../utils/useFetchDataComponent";
...
const PostPropertiesView = () => {
  ...
  } = useFetchDataComponents(formData);

  // Handle the loading and error states
  if (isLoadingState || isLoadingCity || isLoadingBuilder) {
    return <div>Loading...</div>;
  }

  if (stateError || cityError || builderError) {
...
const handleInputChange = (e) => {
  ...
  if (type === "file") {
    ...
    // Dynamically update panels if prop_type changes
    if (name === "prop_type") {
      updatePanels(value);
    }
  }
...
const updatePanels = (propType) => {
  setPanels((prevPanels) => {
    const updatedPanels = [...prevPanels];

    // Clear out the second panel
    updatedPanels[1] = [];

    // Add new fields based on the selected prop_type
    switch (propType) {
      case "3": // Example case for prop_type 3
        updatedPanels[1] = [
          { label: "Unit#", name: "tower_no", type: "text" },
          { label: "Rental Price", name: "rental_low", type: "text" },
          {
      ...
      case "1": // Example case for prop_type 1
        updatedPanels[1] = [
          { label: "Tower#", name: "tower_no", type: "text" },
          { label: "Floor#", name: "floor_no", type: "number" },
          { label: "Flat#", name: "flat_no", type: "text" },
      ...
      default:
        console.warn("Unhandled prop_type value:", propType);
        break;
    }
  ...
  const renderInputs = (inputs) => {
    const dropdownConfig = {
      ...
      city: {
        ...
        disabled: isLoadingCity || !formData.state,
        error: cityError,
        helperText: !formData.state ? "Please select a state first" : "",
        displayKey: "name",
        valueKey: "id",
      },
      builder: {
        ...
        disabled: isLoadingBuilder || !formData.city,
        error: builderError,
        helperText: !formData.city ? "Please select a city first" : "",
        displayKey: "name",
        valueKey: "id",
      },
      community_id: {
        ...
        disabled: isLoadingCommunity || !formData.builder,
        error: communityError,
        helperText: !formData.builder ? "Please select a builder first" : "",
        displayKey: "name",
        valueKey: "id",
      },
      prop_type_id: {
      ...
      home_type_id: {
        options: Array.isArray(homeType) ? homeType : [],
        displayKey: "home_type",
        valueKey: "id",
      },
      no_balconies: {
        options: Array.isArray(balcony) ? balcony : [],
        displayKey: "nbalcony",
        valueKey: "id",
      },
      parking_count_id: {
        options: Array.isArray(parkingCount) ? parkingCount : [],
        displayKey: "parking_count",
        valueKey: "id",
      },
      tenant_type_id: {
        options: Array.isArray(tenant) ? tenant : [],
        displayKey: "tenant_type",
        valueKey: "id",
      },
      tenant_eat_pref_id: {
        options: Array.isArray(tenantEat) ? tenantEat : [],
        displayKey: "eat_pref",
        valueKey: "id",
      },
      prop_desc_id: {
  ...
  <div key={idx} className="col-span-1">
    <label className="block text-black-700 mb-2">{input.label}</label>
    {input.type === "dropdown" && dropdownConfig[input.name] ? (
      <SearchableDropdown
        name={input.name}
        options={dropdownConfig[input.name].options}
        value={formData[input.name]}
        onChange={handleInputChange}
        placeholder={`Select ${input.label}`}
        isLoading={dropdownConfig[input.name].isLoading}
        disabled={dropdownConfig[input.name].disabled}
        error={dropdownConfig[input.name].error}
        helperText={dropdownConfig[input.name].helperText}
        displayKey={dropdownConfig[input.name].displayKey}
        valueKey={dropdownConfig[input.name].valueKey}
      />
    ) : input.type === "file" ? (
      <>
        <input
